    ...
    public static void main(String[] args) {
        ...
        for (DataRow r_tableName : r_tableNames) {
            for (DataColumn c_tableName : r_tableName.GetColumn()) {
                ...
                try {
                    //自动生成实体类
                    (new VelocityHelper(tableName, db_tableColumns, Contants.GENERATE_CARBON + "/" + Contants.GENERATE_PACKAGENAME + "/domain", Contants.GENERATE_PROJECTNAME)).GenerateEntity();
                    //自动生成jpa接口文件
                    (new VelocityHelper(tableName, db_tableColumns, Contants.GENERATE_CARBON + "/" + Contants.GENERATE_PACKAGENAME + "/repository", Contants.GENERATE_PROJECTNAME)).GenerateJpa();
                    //自动生成Service服务类文件
                    (new VelocityHelper(tableName, db_tableColumns, Contants.GENERATE_CARBON + "/" + Contants.GENERATE_PACKAGENAME + "/service", Contants.GENERATE_PROJECTNAME)).GenerateService();
                    //自动生成Controller控制类文件
                    (new VelocityHelper(tableName, db_tableColumns, Contants.GENERATE_CARBON + "/" + Contants.GENERATE_PACKAGENAME + "/controller", Contants.GENERATE_PROJECTNAME)).GenerateController();
